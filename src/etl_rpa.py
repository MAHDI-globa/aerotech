...
# 2) Fichiers: .env.files.{ENV} (fallback .env.files)
...
if files_env_file.exists():
    ...
    raise RuntimeError(
        f"Env fichiers introuvable: {files_env_file} et {fallback_files_env}.\n"
        f"Ajoute p.ex.:\nFILE_PATH_RPA='C:/globasoft/aerotech/fic/Relevé pointages AEB.xlsx'"
    )

FILE_PATH = os.getenv("FILE_PATH_RPA")
if not FILE_PATH:
    raise RuntimeError(f"FILE_PATH_RPA manquant dans {files_env_loaded}")

# ============== CONFIG ==============
SHOW_SAMPLE = True
STRICT_SOURCE_SCHEMA = True
TARGET_SCHEMA = "public"
IF_EXISTS_MODE = "replace"

# ============== LOGGER ==============
logger = get_logger("etl_rpa", logfile="etl_rpa.log")
...
def excel_serial_to_datetime(series: pd.Series) -> pd.Series:
    ...
    lower, upper = 60.0, 60000.0
    mask = (vals >= lower) & (vals <= upper)
    base = pd.Timestamp("1899-12-30")
    return pd.to_datetime(base + pd.to_timedelta(vals.where(mask, np.nan), unit="D", errors="coerce"),
                          errors="coerce")
...
# ============== MAIN ==============
def main():
    with correlation("rpa-"):
        xlsx = Path(FILE_PATH)
        logger.info("Start etl_rpa | file=%s env=%s files-env=%s", xlsx, ENV, getattr(files_env_loaded, "name", "?"))
        if not xlsx.exists():
